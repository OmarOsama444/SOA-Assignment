...
public class PlaceOrderServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            JSONArray products = orderObj.optJSONArray("products");
            if (products != null) {
                for (int i = 0; i < products.length(); i++) {
                    JSONObject p = products.getJSONObject(i);
                    int pid = p.getInt("product_id");
                    p.put("product_name", productIdToName.getOrDefault(pid, "Unknown Product"));
                }
            }
            orderObj.put("customer_name", customerName);

            Map<String, Object> responseData = new HashMap<>();
            responseData.put("order", orderObj.toMap());
